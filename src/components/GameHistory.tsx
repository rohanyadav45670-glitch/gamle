...
function ColorDot({ number }: { number: number }) {
  ...
  if (colors.length === 1) {
    return (
      <div className={cn(
        "w-7 h-7 rounded-full flex items-center justify-center text-white font-black text-[11px] shadow-md flex-shrink-0",
        colors[0] === "red" && "bg-gradient-to-br from-red-500 to-rose-600",
        colors[0] === "green" && "bg-gradient-to-br from-green-500 to-emerald-600",
        colors[0] === "violet" && "bg-gradient-to-br from-violet-500 to-purple-600",
      )}>
        {number}
      </div>
    );
  ...
  // dual color (0 or 5)
  return (
    <div className="w-7 h-7 rounded-full overflow-hidden flex-shrink-0 relative shadow-md">
      <div className={cn(
        "absolute inset-0",
