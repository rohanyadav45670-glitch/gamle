...
import type { ColorChoice, GamePhase } from "@/types/game";

interface BettingPanelProps {
  phase: GamePhase;
  balance: number;
  currentBet: { choice: string; amount: number } | null;
  onPlaceBet: (choice: ColorChoice | "big" | "small", amount: number) => void;
  onCancelBet: () => void;
...
export default function BettingPanel({
  ...
  const [selectedColor, setSelectedColor] = useState<ColorChoice | "big" | "small" | null>(null);
  const [amount, setAmount] = useState(10);
  const isLocked = phase === "locked" || phase === "revealing" || phase === "result";

  function handleConfirm() {
    if (!selectedColor || amount <= 0 || isLocked) return;
    onPlaceBet(selectedColor as ColorChoice | "big" | "small", amount);
  }
          ...
          <div className="flex gap-1.5 items-center flex-wrap">
            {Array.from({ length: 10 }).map((_, i) => (
              <button
                key={i}
                disabled={isLocked || hasBet}
                onClick={() => setSelectedColor(i.toString() as ColorChoice)}
                className={cn(
        ...
        <div className="space-y-2">
          ...
          <div className="bg-white/5 border border-white/10 rounded-2xl px-4 py-3 flex items-center justify-between">
            <div>
              ...
              <p className="text-white font-bold capitalize">{currentBet?.choice} · ₹{currentBet?.amount}</p>
            </div>
            <div className="text-right">
              <p className="text-white/50 text-xs">Potential Win</p>
              <p className="text-green-400 font-bold">
                ₹{((currentBet?.amount ?? 0) * (multipliers[currentBet?.choice ?? "red"] ?? 2)).toFixed(2)}
              </p>
            </div>
          </div>
          {!isLocked && (
